...
def load_pair_artifact(instrument: str):
    ...
    artifact_path = APP_DIR / cfg["artifact"]
    metadata_path = APP_DIR / cfg["metadata"]
    if not artifact_path.exists():
        raise FileNotFoundError(f"Missing artifact file: {artifact_path.name}")
...
st.markdown("---")
st.markdown(
    "Deploy this file as `app.py`. Keep the EUR artifact files (`best_model_artifact.joblib`, `best_model_metadata.json`) together with the uploaded GBP, XAU, AUD, and JPY artifact + metadata files in the same repo, and add `OANDA_API_KEY` in Streamlit secrets."
)
