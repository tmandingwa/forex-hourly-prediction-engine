...
def build_pair_feature_frame(df_mid: pd.DataFrame, instrument: str) -> pd.DataFrame:
    ...
    ny_hour = df["atl_hour"]
    df["is_tokyo_open"] = ((ny_hour >= 19) | (ny_hour < 4)).astype(int)
    df["is_sydney_open"] = ((ny_hour >= 17) | (ny_hour < 2)).astype(int)
    df["is_london_open"] = ((ny_hour >= 3) & (ny_hour < 12)).astype(int)
    df["is_ny_open"] = ((ny_hour >= 8) & (ny_hour < 17)).astype(int)
    df["is_asia_overlap"] = ((df["is_tokyo_open"] == 1) & (df["is_sydney_open"] == 1)).astype(int)
    df["markets_open_count"] = df[["is_tokyo_open", "is_sydney_open", "is_london_open", "is_ny_open"]].sum(axis=1)
    ...
    df[f"{p}_atr_pct"] = df[f"{p}_atr_14"] / (df[close_col] + 1e-9)
    ...
    prev_roll_high_10 = df[f"{p}_rolling_high_10"].shift(1)
    prev_roll_low_10 = df[f"{p}_rolling_low_10"].shift(1)
    df[f"{p}_breakout_up"] = (df[close_col] > prev_roll_high_10).astype(float)
    df[f"{p}_breakout_down"] = (df[close_col] < prev_roll_low_10).astype(float)

    df["dist_to_high_10"] = df[close_col] - df[high_col].rolling(10).max()
    df["dist_to_high_24"] = df[close_col] - df[high_col].rolling(24).max()
    df["dist_to_low_10"] = df[close_col] - df[low_col].rolling(10).min()
    df["dist_to_low_24"] = df[close_col] - df[low_col].rolling(24).min()
    ...
    df[f"{p}_direction_lag_2"] = df[f"{p}_direction"].shift(2)
...
st.markdown("---")
st.markdown(
    "Deploy this file as `app.py`. Keep the local artifact and metadata files in the same folder as the app, or inside `models/`. The consolidated home table is sorted latest first. Default lag settings are EUR=3, GBP=10, XAU=10, AUD=6, JPY=6, but you can adjust them in the sidebar. Default minimum count required is 2, and it is also adjustable in the sidebar."
)
