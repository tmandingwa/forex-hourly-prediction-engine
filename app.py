...
def build_pair_feature_frame(df_mid: pd.DataFrame, instrument: str) -> pd.DataFrame:
    ...
    df[f"{p}_return_lag_2"] = df[f"{p}_return"].shift(2)
    df[f"{p}_return_lag_3"] = df[f"{p}_return"].shift(3)
    df[f"{p}_atr_pct"] = df[f"{p}_atr_14"] / (df[close_col].abs() + 1e-9)
    df[f"{p}_dir_num"] = np.sign(df[f"{p}_direction"]).fillna(0)
    df[f"{p}_dir_lag_1_num"] = np.sign(df[f"{p}_direction"].shift(1)).fillna(0)
    df[f"{p}_dir_lag_2_num"] = np.sign(df[f"{p}_direction"].shift(2)).fillna(0)
    df[f"{p}_breakout_up"] = (df[close_col] > df[high_col].shift(1).rolling(10).max()).astype(float)
    df[f"{p}_breakout_down"] = (df[close_col] < df[low_col].shift(1).rolling(10).min()).astype(float)

    tokyo_open_mask = df["atl_hour"].between(19, 23) | df["atl_hour"].between(0, 3)
    london_open_mask = df["atl_hour"].between(3, 11)
    ny_open_mask = df["atl_hour"].between(8, 16)
    sydney_open_mask = df["atl_hour"].between(17, 23) | df["atl_hour"].between(0, 1)
    df["is_tokyo_open"] = tokyo_open_mask.astype(float)
    df["is_ny_open"] = ny_open_mask.astype(float)
    df["is_sydney_open"] = sydney_open_mask.astype(float)
    df["is_asia_overlap"] = (tokyo_open_mask & sydney_open_mask).astype(float)
    df["markets_open_count"] = (
        tokyo_open_mask.astype(int) + london_open_mask.astype(int) + ny_open_mask.astype(int) + sydney_open_mask.astype(int)
    ).astype(float)
...
with st.sidebar:
    ...
    eur_include_day_of_week = st.checkbox("For EUR only: add day of week as part of key", value=True)
    ...
    for instrument in PAIR_ORDER:
        pair_lag_settings[instrument] = st.slider(
            f"{PAIR_CONFIG[instrument]['display']} lags",
            min_value=1,
            max_value=MAX_NUM_LAGS,
            value=int(PAIR_DEFAULT_LAGS.get(instrument, DEFAULT_NUM_LAGS)),
            step=1,
...
try:
    with st.spinner("Fetching live candles, building probability pipeline, and scoring all ML models..."):
        for instrument in PAIR_ORDER:
            ...
            pair_include_day_of_week = (instrument == "EUR_USD" and eur_include_day_of_week)
            df_pattern = build_pattern_dataset(df_tz, num_lags=pair_num_lags, include_day_of_week=pair_include_day_of_week)
            ...
            final_table = build_final_recommendation_table(instrument, prob_log, ml_log)
...
st.markdown("---")
st.markdown(
    "Deploy this file as `app.py`. Keep the local artifact and metadata files in the same folder as the app, or inside `models/`. Lags stay dynamic in the sidebar with defaults EUR=3, GBP=10, XAU=10, AUD=6, JPY=6. Minimum count required defaults to 2 and is also dynamic. EUR can optionally include day of week in the key. GBP final recommendations use probability only, while the other pairs trade only when probability and ML agree."
)
